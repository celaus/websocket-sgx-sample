...
typedef struct ms_start_thread_t {
	size_t ms_retval;
	int ms_fd;
	char* ms_hostname;
	size_t ms_hostname_len;
	char* ms_cert;
	size_t ms_cert_len;
} ms_start_thread_t;
...
static sgx_status_t SGX_CDECL sgx_start_thread(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_start_thread_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_start_thread_t* ms = SGX_CAST(ms_start_thread_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	char* _tmp_hostname = ms->ms_hostname;
	size_t _len_hostname = ms->ms_hostname_len ;
	char* _in_hostname = NULL;
	char* _tmp_cert = ms->ms_cert;
	size_t _len_cert = ms->ms_cert_len ;
	char* _in_cert = NULL;
	...
	if (_tmp_hostname != NULL && _len_hostname != 0) {
		...
		if (memcpy_s(_in_hostname, _len_hostname, _tmp_hostname, _len_hostname)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

		_in_hostname[_len_hostname - 1] = '\0';
		if (_len_hostname != strlen(_in_hostname) + 1)
		{
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	...
	if (_tmp_cert != NULL && _len_cert != 0) {
		_in_cert = (char*)malloc(_len_cert);
		if (_in_cert == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_cert, _len_cert, _tmp_cert, _len_cert)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

		_in_cert[_len_cert - 1] = '\0';
		if (_len_cert != strlen(_in_cert) + 1)
		{
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	...
	ms->ms_retval = start_thread(ms->ms_fd, _in_hostname, _in_cert);

err:
	if (_in_hostname) free(_in_hostname);
...
SGX_EXTERNC const struct {
	...
	3,
	{
		{(void*)(uintptr_t)sgx_start_thread, 0, 0},
