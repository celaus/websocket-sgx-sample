...
sgx_status_t start_thread(sgx_enclave_id_t eid, size_t* retval, int fd, char* hostname, char* cert)
{
	sgx_status_t status;
	ms_start_thread_t ms;
	ms.ms_fd = fd;
	ms.ms_hostname = hostname;
	ms.ms_hostname_len = hostname ? strlen(hostname) + 1 : 0;
	ms.ms_cert = cert;
	ms.ms_cert_len = cert ? strlen(cert) + 1 : 0;
